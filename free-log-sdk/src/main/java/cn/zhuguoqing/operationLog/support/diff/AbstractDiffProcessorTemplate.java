...
public abstract class AbstractDiffProcessorTemplate implements IDiffProcessor {
  ...
  @Autowired protected IExcludeColumnGetService excludeColumnGetService;

  @Autowired protected ILogInsertAndImportService baseLogInfoService;

  @Autowired protected ModifyColCommentValueFactory modifyColCommentValueFactory;

  /**
   * 在数据改变执行之前调用该方法,一般是SQL执行前
   *
   * @param dto DiffDTO
   * @return Diff的所有信息
   */
   ...
   * 在数据改变后调用该方法
   *
   * @param diffAnyThingDTO Diff的所有信息
   * @param newKeyValue 新的key值,如果一般修改key值没有改变就不用修改
   */
  @Override
  public void afterUpdate(DiffAnyThingDTO diffAnyThingDTO, String... newKeyValue) {
    ...
    try {
      ...
      if (newKeyValue == null || newKeyValue.length == 0) {
        ...
        sql =
            appendSQL(diffAnyThingDTO.getColumns(), diffAnyThingDTO.getDiffDTO(), newKeyValue)
                + " for update ";
      }
  ...
  /**
   * 数据插入之后记录日志信息,一般只用于插入的操作
   *
   * @see SingleAddDomainDiffProcessor
   */
  @Override
  ...
  /** 构造DiffAnyThingDTO */
  protected DiffAnyThingDTO buildDiffAnyThingDTO(
      List<String> colums,
  ...
  protected String getClmComment(String comment, String key, DiffDTO dto) {
    ...
    if (!CollectionUtils.isEmpty(customKeyFunctionMap) && customKeyFunctionMap.containsKey(key)) {
      ...
      IModifyColCommentValueService service =
          modifyColCommentValueFactory.getService(CustomFunctionType.KEY, key);
      if (service != null) {
  ...
  protected String getClmValue(String value, String key, DiffDTO dto) {
    ...
    if (!CollectionUtils.isEmpty(customValueFunctionMap)
        && customValueFunctionMap.containsKey(key)) {
      ...
      IModifyColCommentValueService service =
          modifyColCommentValueFactory.getService(
              CustomFunctionType.VALUE, schemaTableName + "." + key);
      if (service != null) {
  ...
  /** 设置字段注释 */
  protected void setClmComment(
      OperationLogDetailDomain opd, String key, String comment, DiffDTO dto) {
    Map<String, Function<String, String>> customKeyFunctionMap = dto.getCustomCommentFunctionMap();
    if (!CollectionUtils.isEmpty(customKeyFunctionMap) && customKeyFunctionMap.containsKey(key)) {
      Function<String, String> function = customKeyFunctionMap.get(key);
      ...
      IModifyColCommentValueService service =
          modifyColCommentValueFactory.getService(
              CustomFunctionType.KEY, schemaTableName + "." + key);
      if (service != null) {
  ...
  /** 设置字段值 */
  protected void setClmValue(
      OperationLogDetailDomain opd, String key, String oldValue, String newValue, DiffDTO dto) {
    Map<String, Function<String, String>> customValueFunctionMap = dto.getCustomValueFunctionMap();
    if (CollectionUtils.isEmpty(customValueFunctionMap)
        || !customValueFunctionMap.containsKey(key)) {
      String schemaTableName = dto.getSchemaTableName();
  ...
  /** 获取字段的注释 */
  protected String getSimgeClmValueBySingleParam(
      List<Map<String, Object>> paramMap, String column) {
    if (paramMap.size() >= 2) {
