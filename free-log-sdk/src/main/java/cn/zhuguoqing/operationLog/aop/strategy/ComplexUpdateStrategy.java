...
/**
 * @author guoqing.zhu
 *     <p>description:复杂修改数据的策略,这里可以记录旧值新值
 */
@Component
@Slf4j
public class ComplexUpdateStrategy extends AbstractOperationLogStrategy {
  ...
  public Object operate(ProceedingJoinPoint p, OperationLog operationlog) {
    // 0.仔细思考没必要使用原子boolean,栈都是线程独享的,先留着把,可以再优化;
    ...
    try {
      ...
      // 3.获取要解析的模板
      List<String> beforeSpElTemplates = getProcessBeforeExecuteTemplate(operationLogDTO);
      // 4.获取要解析的模板
      List<String> spElTemplates = getBeforeExecuteFunctionTemplate(operationLogDTO);
      // 5.解析procceed之前的spEL模板,主要用于获取参数
      Map<String, String> templateReturnMap =
          processBeforeExecuteTemplate(
              beforeSpElTemplates, getTargetClass(p.getTarget()), getMethod(p), p.getArgs());
      // 6.解析procceed之前的spEL模板,主要用于获取自定义方法
      ...
      // 8.如果注解的cloum属性没有值,那么就取数据库的值
      cloum = operationlog.includeColumn();
      String[] excludeColumn = operationlog.excludeColumn();
      cloum = getColumnNeedRecord(columnCommentMap, cloum, excludeColumn);
      // 9.拼接根据id获取到旧值;
      assembleSQL =
          operationLogService.getAssembleSQL(
              operationLogDTO.getTable(),
              operationLogDTO.getIdName(),
              templateReturnMap.get(operationLogDTO.getIdRef()),
              cloum);
    ...
    try {
      if (atomicBoolean.get()) {
        // 12.切点执行完,表示数据库已经更新完成,执行相同的SQL查询修改的数据
        Map<String, Object> newMap = iOperationLogService.selectAnyTable(assembleSQL);
        // 13.执行记录
        RecordExecutorDTO recordExecutorDTO =
            RecordExecutorDTO.builder()
                .oldMap(oldMap)
                .newMap(newMap)
                .ret(proceed)
                .method(getMethod(p))
    ...
    try {
      ...
      Map<String, String> expressionValues =
          processTemplate(
              ...
              dto.getArgs(),
              dto.getErrorMsg(),
              dto.getFunctionNameAndReturnMap());
      // condition is true
      if (logConditionPassed(dto.getOperationLogDTO().getCondition(), expressionValues)) {
        Operator operator = getOperator();
        operationLogService.insertLogAndLogDetail(
            dto.getSchemaTable(),
            dto.getOldMap(),
            dto.getNewMap(),
            dto.getOperationLogDTO(),
            dto.getCloum(),
            dto.getColumnCommentMap(),
            expressionValues,
            action,
            operator,
            dto.getSuccess());
